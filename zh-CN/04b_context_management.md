...
**先建立直觉**：AI 模型有一个"记忆窗口"（上下文窗口），能记住的内容是有限的。Claude Code 需要在这个有限的窗口里塞入最有价值的信息。整个上下文管理分三个阶段：开始时**注入**必要背景（项目规则、git 状态等），运行中**按需披露**（用到什么才加载什么），对话变长时**压缩**旧内容（从便宜到贵，逐层压缩）。
...
**先建立直觉**：每次你打开 Claude Code 开始新会话时，系统会自动收集两类"背景信息"注入给 AI：一是你的项目规则（CLAUDE.md 文件），二是当前 git 状态。这些信息只在会话开始时收集一次（用缓存避免重复计算），就像你每天上班时看一眼日程表，而不是每分钟都看一次。
