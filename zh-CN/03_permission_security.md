...
**先建立直觉**：Claude Code 让 AI 直接操作你的电脑——读写文件、执行命令、访问网络。这就像把家门钥匙交给一个非常聪明但偶尔会犯错的助手。权限系统就是给这个助手设定的规矩：哪些房间可以自由进出，哪些需要先问你，哪些绝对不能碰。
...
**先建立直觉**：当 AI 想执行一个操作时，系统会经过一系列"关卡"来决定是否放行。这个流程就像机场安检——先查黑名单（直接拒绝）、再查可疑名单（需要人工检查）、然后看你的票（权限模式）、最后才放行。每一层都可能拦截请求，越往后越宽松。
...
**先建立直觉**：权限规则就像一份"白名单/黑名单"清单。你可以在不同层级设置规则——全局的（所有项目生效）、项目级的（只对当前项目生效）、甚至临时的（只在当前会话生效）。规则的格式很直观：`"Bash"` 表示所有 shell 命令，`"Bash(npm install)"` 表示只匹配这一条命令。
