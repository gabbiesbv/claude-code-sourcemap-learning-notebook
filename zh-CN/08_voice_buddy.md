...
**先建立直觉**：Voice 模式让你可以按住快捷键说话，Claude Code 会把语音转成文字输入。整个流程是：按住键 → 录音 → 通过 WebSocket 发送到语音识别服务 → 识别结果自动填入输入框。但这个功能不是所有人都能用——它需要通过三层检查（编译时的功能开关、OAuth 认证、远程 kill-switch），任何一层不通过就不可用。
