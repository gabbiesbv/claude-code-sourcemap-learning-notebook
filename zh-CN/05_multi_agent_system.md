...
**先建立直觉**：有时候一个任务太大，一个 AI 忙不过来。Claude Code 的解决方案是"分身术"——主 AI 可以创建子 Agent 来并行处理子任务。就像一个项目经理把大任务拆分给团队成员：有的同步等待结果（像面对面交代任务），有的异步后台运行（像发邮件布置任务），还有的共享上下文（像在同一个文档里协作）。
...
**先建立直觉**：当主 AI 创建一个子 Agent 时，子 Agent 默认拥有自己独立的"工作空间"——独立的文件缓存、独立的记忆、独立的取消控制器。如果需要共享某些东西（比如让子 Agent 能弹出权限对话框），必须**显式声明**。这就像公司给新员工分配独立工位，但如果需要访问共享打印机，得单独申请权限。
